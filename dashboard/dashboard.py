...
def load_data():
    print(os.getcwd())
    df_agg = pd.read_csv('dashboard/data/Aggregated_Metrics_By_Video.csv').iloc[1:,:]
    ...
    df_agg_sub = pd.read_csv('dashboard/data/Aggregated_Metrics_By_Country_And_Subscriber_Status.csv')
    df_comments = pd.read_csv('dashboard/data/Aggregated_Metrics_By_Video.csv')
    df_time = pd.read_csv('dashboard/data/Video_Performance_Over_Time.csv')
...
if add_sidebar == 'Aggregate Metrics':
    ...
    # st.metric('Views', metric_medians6mo['Views'], 450)
    ...
    # st.dataframe(df_agg_diff_final.style.hide().applymap(style_negative, props='color:red').applymap(style_positive, props='color:green'))
...
if add_sidebar == 'Individual Video Analysis':
    ...
    fig2.add_trace(go.Scatter(x=views_cumulative['days_published'], y=views_cumulative['median_views'],
                        mode='lines',
                        name='50th percentile', line=dict(color='teal', dash ='dash')))
